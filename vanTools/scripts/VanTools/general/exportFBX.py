...
def main():
    ...
    if selectionList:
        myWindow = ExportWindow(selectionList, mayaName,
                                pathFile, prefFile)  # createUI
        ...
        with pm.window(title="Export FBX", toolbox=True, sizeable=False, topLeftCorner=(200, 320), w=490, h=self.scrollHeight) as self.window:
            with pm.formLayout() as self.form:
                ...
                with pm.rowLayout(numberOfColumns=3, adjustableColumn=2, ) as self.row:
                    pm.text(label="Path:")
                    self.pathField = pm.textField(
                        searchField=True, text=self.path)
                    pm.button("...", c=self.changePath, w=50)

                self.exportBut = pm.button(
                    "Export", c=self.export, w=50, bgc=(0.28, 0.66, 0.71))
        ...
        if self.path:
            ...
            if centerExport:
                ...
                for s in self.selectionList:
                    ...
                    if "|" in s:
                        s = s.split("|")[-1]
                    pm.exportSelected(self.path+'\\'+str(s) +
                                      ".fbx", f=True)  # Export FBXs
            ...
            confirm = pm.confirmDialog(title="Success", b=[
                                       "Done", "Open Folder"], message="Exported:\t\t\n\n" + self.printList, messageAlign="left")
